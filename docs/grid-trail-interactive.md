# Grid-trail interactive glow (archived)

The homepage hero and “Take the Next Step” CTA sections still show a **static** white grid (via `scripts/hero-grid-interactive.js`). The **animated** red cell-glow that followed the mouse (desktop) or scroll (mobile) was removed after feedback, but the full script is preserved below so you can turn it back on later.

## Where it appears

- Homepage hero: `.hero.grid-trail` + `<canvas class="grid-trail-canvas">`
- “Take the Next Step” banners site-wide: `.cta-banner.grid-trail` + same canvas
- Script tag on those pages: `scripts/hero-grid-interactive.js`

## How to restore the animation

1. Replace the entire contents of [`scripts/hero-grid-interactive.js`](../scripts/hero-grid-interactive.js) with the archived script in the code block below.
2. Redeploy / hard-refresh. No HTML or CSS changes are required — the canvas markup and `.grid-trail` styles are unchanged.
3. Optional: remove or shorten this doc once the animation is live again.

Accessibility notes baked into the interactive version:

- Bails out when `prefers-reduced-motion: reduce` is set
- CSS already hides `.grid-trail-canvas` under that media query in `styles.css`

## Archived interactive script
